...
with st.sidebar:
    ...
    with st.expander("🏚️ Scenario A: Old Machine"):
        ...
        old_kwh = st.number_input("Old Energy (kWh/unit)", value=45.0, step=1.0)
        old_def = st.number_input("Old Defect Rate (%)", value=8.0, step=0.5)
        old_maint_L = st.number_input("Old Maint. (₹ Lakhs/Yr)", value=45.0, step=1.0)
        old_labor_L = st.number_input("Old Labor (₹ Lakhs/Yr)", value=30.0, step=1.0)

    with st.expander("✨ Scenario B: New Machine"):
        new_capex_L = st.number_input("New CapEx (₹ Lakhs)", value=350.0, step=10.0)
        new_kwh = st.number_input("New Energy (kWh/unit)", value=18.0, step=1.0)
        new_def = st.number_input("New Defect Rate (%)", value=1.5, step=0.5)
        new_maint_L = st.number_input("New Maint. (₹ Lakhs/Yr)", value=8.0, step=1.0)
        new_labor_L = st.number_input("New Labor (₹ Lakhs/Yr)", value=12.0, step=1.0)
        new_salvage_L = st.number_input("End Salvage (₹ Lakhs)", value=50.0, step=5.0)
        
        st.caption("Financing:")
        debt_pct = st.number_input("Debt Funded (%)", value=70.0, step=5.0)
        debt_rate = st.number_input("Interest Rate (%)", value=9.0, step=0.5)
        debt_term = st.number_input("Loan Term (Yrs)", value=5, step=1)

# --- VALUE CONVERSION (Lakhs to Raw ₹) ---
old_salvage = old_salvage_L * 100000
old_maint = old_maint_L * 100000
old_labor = old_labor_L * 100000

new_capex = new_capex_L * 100000
new_maint = new_maint_L * 100000
new_labor = new_labor_L * 100000
new_salvage = new_salvage_L * 100000

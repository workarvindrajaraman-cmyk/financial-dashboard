...
# ============================================================
# TAB 5 — NATIVE EXCEL EXPORT ENGINE
# ============================================================
with tab5:
    st.info("📥 Export the fully-linked three-statement model, KPIs, DCF, sensitivity matrix (with heatmaps), and native Excel charts.")
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # --- Write Data ---
        df_master.to_excel(writer, sheet_name='Financial Statements')
        writer.sheets['Financial Statements'].set_column('A:A', 25)
        writer.sheets['Financial Statements'].set_column('B:Z', 15, num_fmt)
        
        kpi_df.to_excel(writer, sheet_name='Advanced KPIs')
        writer.sheets['Advanced KPIs'].set_column('A:A', 25)
        writer.sheets['Advanced KPIs'].set_column('B:Z', 15, num_fmt)
        
        pd.DataFrame(dcf_data).set_index("Metric").to_excel(writer, sheet_name='DCF Valuation')
        writer.sheets['DCF Valuation'].set_column('A:A', 30)
        writer.sheets['DCF Valuation'].set_column('B:B', 20, num_fmt)
        
        # --- Sensitivity & Heatmaps ---
        sensitivity_ev.to_excel(writer, sheet_name='Sensitivity - EV')
        ws_ev = writer.sheets['Sensitivity - EV']
        ws_ev.set_column('A:A', 15)
        ws_ev.set_column('B:Z', 15, num_fmt)
        ws_ev.conditional_format(1, 1, len(sensitivity_ev.index), len(sensitivity_ev.columns),
                                 {'type': '3_color_scale', 'min_color': '#fca5a5', 'mid_color': '#cbd5e1', 'max_color': '#86efac'})
        
        sensitivity_price.to_excel(writer, sheet_name='Sensitivity - Price')
        ws_pr = writer.sheets['Sensitivity - Price']
        ws_pr.set_column('A:A', 15)
        ws_pr.set_column('B:Z', 15, num_fmt)
        ws_pr.conditional_format(1, 1, len(sensitivity_price.index), len(sensitivity_price.columns),
                                 {'type': '3_color_scale', 'min_color': '#fca5a5', 'mid_color': '#cbd5e1', 'max_color': '#86efac'})
        
        # --- Visual Dashboards (Native Charts) ---
        ws_charts = workbook.add_worksheet('Visual Dashboards')
        num_years = len(years)
        
        # Chart 1: Rev vs EBITDA
        chart1 = workbook.add_chart({'type': 'column'})
        rev_row = df_master.index.get_loc('Revenue') + 1
        ebitda_row = df_master.index.get_loc('EBITDA') + 1
        
        chart1.add_series({
            'name': ['Financial Statements', rev_row, 0],
            'categories': ['Financial Statements', 0, 1, 0, num_years],
            'values': ['Financial Statements', rev_row, 1, rev_row, num_years],
            'fill': {'color': '#00b050'}
        })
        chart1.add_series({
            'name': ['Financial Statements', ebitda_row, 0],
            'categories': ['Financial Statements', 0, 1, 0, num_years],
            'values': ['Financial Statements', ebitda_row, 1, ebitda_row, num_years],
            'fill': {'color': '#3b82f6'}
        })
        chart1.set_title({'name': f'Revenue vs EBITDA ({unit_suffix})'})
        ws_charts.insert_chart('B2', chart1, {'x_scale': 1.5, 'y_scale': 1.2})
        
        # Chart 2: FCF vs OCF
        chart2 = workbook.add_chart({'type': 'line'})
        fcf_row = df_master.index.get_loc('Free Cash Flow') + 1
        ocf_row = df_master.index.get_loc('Operating CF') + 1
        
        chart2.add_series({
            'name': ['Financial Statements', fcf_row, 0],
            'categories': ['Financial Statements', 0, 1, 0, num_years],
            'values': ['Financial Statements', fcf_row, 1, fcf_row, num_years],
            'line': {'color': '#00b050', 'width': 2.5}
        })
        chart2.add_series({
            'name': ['Financial Statements', ocf_row, 0],
            'categories': ['Financial Statements', 0, 1, 0, num_years],
            'values': ['Financial Statements', ocf_row, 1, ocf_row, num_years],
            'line': {'color': '#f59e0b', 'width': 2.5}
        })
        chart2.set_title({'name': f'Cash Flow Trends ({unit_suffix})'})
        ws_charts.insert_chart('K2', chart2, {'x_scale': 1.5, 'y_scale': 1.2})
        
        # Chart 3: ROE/ROCE
        chart3 = workbook.add_chart({'type': 'column'})
        roe_row = kpi_df.index.get_loc('ROE (%)') + 1
        roce_row = kpi_df.index.get_loc('ROCE (%)') + 1
        
        chart3.add_series({
            'name': ['Advanced KPIs', roe_row, 0],
            'categories': ['Advanced KPIs', 0, 1, 0, num_years],
            'values': ['Advanced KPIs', roe_row, 1, roe_row, num_years],
            'fill': {'color': '#00b050'}
        })
        chart3.add_series({
            'name': ['Advanced KPIs', roce_row, 0],
            'categories': ['Advanced KPIs', 0, 1, 0, num_years],
            'values': ['Advanced KPIs', roce_row, 1, roce_row, num_years],
            'fill': {'color': '#3b82f6'}
        })
        chart3.set_title({'name': 'Return on Capital (%)'})
        ws_charts.insert_chart('B20', chart3, {'x_scale': 1.5, 'y_scale': 1.2})
